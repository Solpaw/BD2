...
public class EditRaceController {
    ...
    public void updateRace(ActionEvent event) {
        ...
        if(!race.getLocation().equals(location)) {
            race.setLocation(location);
            changed = true;
        }
        if(!race.getRoute().equals(route)){
            race.setRoute(route);
            changed = true;
        }
        if(race.getDateRace().compareTo(Date.valueOf(datePicker.getValue()))!=0){
    ...
    public void setRace(Race race) {
        ...
        cityBox.getSelectionModel().select(race.getLocation().getLocationCity());
        streetBox.getSelectionModel().select(race.getLocation().getLocationStreet());
        lenBox.getSelectionModel().select(Integer.toString(race.getRoute().getRouteLength()));
        obsBox.getSelectionModel().select(Integer.toString(race.getRoute().getRouteObstacles()));
        int i=0;
        for(Object o:priceBox.getItems()) {
            if(race.getRoute().getPrice().getPriceValue()==((Price)o).getPriceValue()){
